  ...
  return (
    <Card className="card-hover flex h-full flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        <Badge tone="accent">{note.subject}</Badge>
        {note.course_code ? <Badge>{note.course_code}</Badge> : null}
        {note.semester ? <span className="text-xs text-ink-muted">{note.semester}</span> : null}
      </div>

      <div>
        <h3 className="text-lg font-semibold text-navy-900">{note.title}</h3>
        <p className="mt-1.5 line-clamp-2 text-sm text-ink-muted">{note.description}</p>
      </div>
      ...
      <div className="mt-auto space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ActionForm
            ...
            className={note.file_url ? "btn-primary" : "btn-quiet"}
          />
          {isOwner ? (
            <ActionForm
